...
Threads cannot be synchronized across the whole grid (between all the threads), only across a block, therefore a new kernel must be launched. Three kernels are used to accomplish this: 

\begin{itemize}
	\item \verb|detectSurvivorsInit| - an initial kernel, which is passed a finishing stage of the classifier and outputs surviving samples.
	\item \verb|detectSurvivors| - an intermediate kernel, which processes surviving samples and outputs surviving samples starting and finishing at given stages. Multiple instances are run.
	\item \verb|detectDetections| - a final kernel starting at a given stage and processing surviving samples until the final stage of the classifier to output detections.
\end{itemize}
...
To solve these issues a method exploiting the use of shared memory was proposed. It is very similar to \ref{subsec:afgm}, also using three types of functions: initial, intermediate and final, but these are wrapped in a single kernel.

After processing a given number of classifier stages threads are synchronized and assigned an ID within their block. A function - intermediate or final is then called. This way only samples at the start of the shared memory are processed.
